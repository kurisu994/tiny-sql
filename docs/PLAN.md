...
**v0.3 开发已于 2026-08-20 完成全部编码与自动化门禁（Week 1-6 + V3-T7.3 文档 + V3-T7.4 门禁）**：独占 session 可靠事务（FR-244）、服务端筛选排序分页（FR-242）、index/constraint 元数据树与对象搜索（FR-241）、多语句执行与 SQL 格式化（FR-243）、SQL 文件工作流（FR-240）。门禁事实：`just check` 全绿（db-driver 单测 35、app_lib 46、ssh-multihop 8、前端 vitest 102、Next build）；双 driver integration 20/20（MySQL 11 含事务/浏览/metadata/多语句、PG 9）；本机 dmg + updater 签名产物构建成功。V3-T7.1 双 driver × 直连/1 跳/3 跳全功能真实回归已于 2026-08-21 用户实测通过，V3-T7.2 RC 一周试用已于 2026-08-22 关闭（0 数据丢失 / 0 凭据泄露 / 0 不可恢复 crash）；剩余仅为 RC 下载安装验收收尾与正式发布。
...
- [x] RC 反馈产生的 v0.2.1 补丁：RC 至正式发布期间无 P0/P1 反馈，无需补丁版本；后续真实用户反馈按常规流程走 v0.2.x 补丁，不混入 v0.3 承诺。
- [x] `REQUIREMENTS.md` 已补 v0.3 范围章节（2026-08-20，§3.3）。

**V3-CP0 已于 2026-08-20 全部收口，v0.3 Week 1 可以开工。**
...
- [x] **V3-T7.2 [3h]** 作者 + 至少 2 位试用者使用 v0.3 RC ≥ 1 周（2026-08-22 关闭）：沿用 V2-T8.2 标准验收通过，0 数据丢失、0 凭据泄露、0 不可恢复 crash。
