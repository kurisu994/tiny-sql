...
v0.4  🚧 编码与自动化门禁完成，rc1 试用中 ── 安全表格编辑 + 对象管理 + CSV/SQL dump 导入导出
   │
   ▼
v0.5  📝 草案 ── 修改表 / 索引设计器 + 官方备份恢复 + 加密分享连接
   │
   ▼
v0.6+ ── 结构同步 + 用户权限 + ER/BI/AI + 平台与 crate 长期演进
```
...
### v0.5 — 结构变更、官方备份与连接协作（草案）

详细任务、依赖、检查点与测试矩阵见 [v0.5 开发计划](./PLAN.md#v05-开发计划)。

**当前进度**：2026-08-24 立项草案；功能代码待 V5-CP0（v0.4.0 正式发布）后开工。
...
| ID | 功能 | 优先级 | 范围边界 |
|---|---|---|---|
| FR-220 | 多集群 diff（同一表在 prod/staging 的 schema 差异） | P1 | SRE 友好；是 FR-261 的前置，不在 v0.5 |
| FR-261 | 数据传输、数据同步与结构同步 | P1 | 基于 FR-220 生成可审阅脚本，默认不自动执行破坏性变更 |
...
以下工程与产品工作在 v0.3-v0.6+ 期间按反馈穿插，不与上面的 Navicat 日常替代主线抢 P0：
...
| ID | 功能 | 优先级 | 备注 |
|---|---|---|---|
| FR-220 | 多集群 diff（同一表在 prod/staging 的 schema 差异） | P1 | 已列入 v0.6+，作为 FR-261 前置 |
