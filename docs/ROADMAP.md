...
v0.6  🚧 编码完成，待 GUI/RC 验收 ── 双连接结构 diff + 可审阅结构同步脚本 + 只读 ER
   │
   ▼
v0.7  🚧 编码完成，待 GUI/RC 验收 ── 表级数据拷贝 + MySQL 权限 + EXPLAIN 可读化
   │
   ▼
v0.8  📝 草案 ── 连接只读 + 环境色 + 复制新表 / 单元格与 FK / RENAME / EXPLAIN 提示
   │
   ▼
...
### v0.7 — 表数据搬迁、库内权限与执行计划（编码完成，待验收）

详细任务历史见 [progress.md v0.7 归档](../memory-bank/progress.md#v07-已交付周计划归档)；收口见 [PLAN](./PLAN.md)。

**当前进度**：2026-08-24 三项 FR 编码完成；待 GUI/RC。

| ID | 功能 | 优先级 | 范围边界 |
|---|---|---|---|
| FR-266 | 双连接表级数据拷贝 | P0 | 同方言、已打开连接；分页读 + bulk_insert；默认追加；清空目标须手输表名 |
...
**当前进度**：2026-08-24 立项，范围已确认；功能代码未开工。

| ID | 功能 | 优先级 | 范围边界 |
|---|---|---|---|
| FR-270 | 连接只读开关 | P0 | 应用层拒绝写；不替代 DB 账号权限 |
| FR-271 | 连接环境色 / 标签 | P1 | prod/staging/dev 展示；不按环境自动只读 |
| FR-272 | 同库复制为新表 | P1 | 预览建表 SQL；可选灌数；手输新表名 |
| FR-273 | 单元格检查器 + 外键跳转 | P1 | 不在检查器写回；跨连接 FK 不做 |
| FR-274 | RENAME COLUMN 预览 | P1 | 可整项降级；仍不换主键 |
| FR-275 | EXPLAIN 读后提示 | P1 | 只标注危险节点；不做监控采集 |

### v0.9+ — 双向同步与高级工作台

| ID | 功能 | 优先级 | 范围边界 |
|---|---|---|---|
| FR-261c | 双向 / 行级同步与冲突合并 | P1 | 不自动执行破坏性变更 |
| FR-264 | BI 图表与仪表板 | P2 | 面向查询结果的本地可视化 |
| FR-265 | Schema-aware AI Assistant | P2 | 发送 schema / SQL 前必须提示数据边界 |

以下工程与产品工作在 v0.3-v0.8+ 期间按反馈穿插，不与上面的 Navicat 日常替代主线抢 P0：
...
| FR-222 | EXPLAIN 可视化 | P1 | 已列入 v0.7（树展示，非监控采集） |
| FR-223 | 慢查询监控 | P2 | 同上 |
