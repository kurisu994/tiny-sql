...
原发布时间预期为 2026-08 月初；截至 2026-08-18，v0.0.3 预览版已完成全平台 Release，用户提交 `4f54f02` 已确认 CP-3/CP-4/CP-5/CP-6 与同事试用完成，但仓库/远端仍无 `v0.1.0-rc1` / `v0.1.0` tag，应用版本仍为 `0.0.3`，README GIF 与少量 v0.1 承诺缺口仍待收口。因此正式发布继续延期，暂不重新承诺日期。

---

## v0.2 — 首发后 2-3 个月

启动条件：现有 dogfooding 验收不重做；先按 [PLAN §13.1](./PLAN.md#13-v02-开发计划) 补发 v0.1.0，并完成全平台安装包与应用内更新验收。发布后的稳定时长和社区反馈用于调整 P2 优先级，不再阻塞 v0.2 Week 1。

详细任务、依赖、检查点与测试矩阵见 [PLAN §13](./PLAN.md#13-v02-开发计划)。
