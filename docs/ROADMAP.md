...
```
v0.1  (5-6 周) ── MySQL + 3 跳 SSH + 拓扑图 + macOS only
   │
   ▼
v0.2  (首发后 2-3 个月) ── PG driver + 自动更新 + passphrase 加密 + TLS + Schema-aware 智能联想
   │
   ▼
...
| **FR-101** | 自动更新（tauri-plugin-updater） | P0 | 0.5 周 |
...
- 加 GitHub Actions release job：Apple Developer 账号签名（$99/年）
