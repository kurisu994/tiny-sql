...
- 成功时同时给出耗时：主行是延迟（握手完成后在热连接上跑 `SELECT 1` 的往返），次行分段列出 SSH 隧道建链、建立连接与总计，便于判断慢在哪一段。走隧道才有隧道段。
- 带口令私钥时，SSH 页提供“私钥 passphrase（仅测试）”输入；`connection_test` 只把它用于本次握手，不写入连接配置，也不进入正式连接的会话缓存。
- **验收标准**：
  - 配置正确 → 5s 内显示成功，并带上延迟与分段耗时。
  - SSH 第 2 跳故意填错端口 → 30s 内显示"第 2 跳连接失败"。
  - MySQL 密码错 → 显示"MySQL 认证失败"，不能误报 SSH 错误。

**FR-003 [P1] 连接列表排序**

- 连接列表按用户手动顺序排列：按住列表项上下拖动即可调整位置，松手落位，顺序随连接配置持久化。
- 从未参与过手动排序的连接（旧记录、新建、导入）排在已排序的连接之后，彼此之间按"最近使用时间"倒序，从未使用的排最后。
- **验收标准**：把连接 A 拖到列表顶部，重启应用后 A 仍在顶部；双击连接 B 建立连接后回到列表，顺序不变。
