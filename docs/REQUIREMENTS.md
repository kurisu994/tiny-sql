...
> **实现快照（2026-08-24）**：v0.3.0 已正式发布；v0.4 三项 FR 编码与自动化门禁全部完成，`v0.4.0-rc1` 已发布（prerelease），待真实环境回归与正式发布。v0.5 范围见 §3.5（草案，待 v0.4.0 与用户确认后生效）。后续版本以 [ROADMAP.md](./ROADMAP.md) 为准，实施计划以 [PLAN.md](./PLAN.md) 为准。
...
> **实现状态（2026-08-24）**：草案已立项，功能代码未开工；待 V5-CP0（v0.4.0 正式发布 + 无阻塞补丁）。
...
实施顺序、验收门槛与降级规则见 [v0.5 开发计划](./PLAN.md#v05-开发计划)。
