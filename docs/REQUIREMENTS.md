...
- 没有 $99 Apple Developer 账号也能用（v0.1 不签名，README 教"右键打开"）。
...
- v0.1 **不签名**：README 顶部写"右键打开 → 允许"操作说明 + GIF。
- **验收标准**：
...
- **FR-101** 自动更新（tauri-plugin-updater）
...
**NFR-014 仅本地通信**：tiny-sql **不联网**（除了用户配置的 SSH/MySQL 目标）。无遥测、无更新检查（v0.1）、无错误上报。
...
- **自动更新**：v0.1 用户手动从 GitHub Releases 下载新版本。理由：避免 $99/年 Apple Developer 账号阻塞首发（v0.2 加 tauri-plugin-updater）。
...
| v0.1 不签名 | $99/年阻塞首发；README 教用户右键打开 |
...
- [ ] tag v0.1.0 推送后 GitHub Releases 自动出现 .dmg
