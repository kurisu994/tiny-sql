## 历史归档（activeContext · 2026-08）

> 2026-08 之前累积到 activeContext.md 的历史轮次、已实现能力、已知代码/承诺边界与近期决策。核心里程碑见 [[progress]]，架构/模式见 [[systemPatterns]]。
...
**上一轮：暗色底改为设计色 #181818（2026-08-28）**——用户给的色卡取样为 rgb(24,24,24)。`.dark` 的 `--background` / `--sidebar` / `--color-neutral-950` 与 SQL 编辑器底都改成 `#181818`，避免窗口是炭黑、局部 `neutral-950` 仍是纯黑。
...
新增回归测试「open 收尾不冲掉连接窗口内已到达的 SSH RTT 采样」（挂起 `db_list_databases` 模拟窗口注入事件）。门禁：vitest 53/53、`just lint`（tsc + clippy）与 `cargo fmt --check` 全绿。**待用户 GUI 实测**：多跳连接后约 1~5 秒内横线上方应出现「SSH xx ms」。

**上一轮：连接链路图紧凑化 + TOFU 弹窗层级修复（2026-08-28）**——两个 UI 反馈：
...
- v0.8 不搬双向同步 / BI / AI（改挂 v0.9+）。唯一不降级 P0 是连接只读开关；只读是应用层，后端写 command 必须认。环境色不自动变只读。
