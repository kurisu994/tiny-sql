## 历史归档（progress · 2026-08）

> 2026-08 之前累积到 progress.md 的已交付周计划明细、开发阶段完成度、Git 提交历史、已解决阻碍表与旧重大决策明细。版本发布历史表与近期风险仍在 [[progress]] 枢纽内。

## v0.8 已交付周计划（归档）

> 2026-08-24 从 `docs/PLAN.md` 移出已完成周计划。应用版本号仍为 `0.7.0`，功能在 main。

| 周 | 内容 | 检查点 | 状态 |
|---|---|---|---|
| Week 1-2 | FR-270 `readOnly` 落盘 + 写 command / ANALYZE 拒绝；表单与写入口禁用 | V8-CP1 / CP2 | ✅ 2026-08-24 |
| Week 3 | FR-271 `env` none/prod/staging/dev；列表圆点与顶栏 | — | ✅ 2026-08-24 |
| Week 4 | FR-272 同库复制为新表（MySQL LIKE / PG 重建 DDL） | V8-CP3 | ✅ 2026-08-24 |
| Week 5-6 | FR-273 单元格检查器 + 同连接 FK 跳转 | V8-CP4 | ✅ 2026-08-24 |
| Week 7 | FR-274 非主键 RENAME COLUMN；FR-275 EXPLAIN 树提示 | — | ✅ 2026-08-24 |
| Week 8 | 文档 + `just check`（vitest 168 / app_lib 67） | — | ✅ 2026-08-24 |
| Week 8 剩余 | GUI 回归（T8.1）+ RC 试用（T8.2）+ 正式发布 | V8-CP5 | ⏳ |

V8-CP0 启动准入按用户要求跳过。v0.8 未新增 Tauri command。
...
| 周 | 内容 | 检查点 | 状态 |
|---|---|---|---|
| Week 1 | FR-220 `diffSchemas` 纯函数 | V6-CP1 | ✅ 2026-08-24 |
| Week 2-3 | 对比台 + 多连接保持打开 + 报告导出 | V6-CP2 | ✅ 2026-08-24 |
| Week 4-5 | FR-261 同步 SQL 生成 / 保存 / queryMany 执行 | V6-CP3 | ✅ 2026-08-24 |
| Week 6-7 | FR-263 只读 ER | V6-CP4 | ✅ 2026-08-24 |
| Week 8 | 文档 + `just check`（vitest 153） | — | ✅ 2026-08-24 |
| Week 8 剩余 | GUI / RC / 正式发布 | V6-CP5 | ⏳ |
