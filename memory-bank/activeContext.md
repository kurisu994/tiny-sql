...
**v0.1.0 已于 2026-08-18 正式发布。** v0.1 实现、dogfooding、MySQL 5.7、作者与同事试用、全平台构建、签名更新产物、`latest.json` 和应用内升级闭环均已完成。v0.2 已形成 8 周实施计划，PostgreSQL 版本基线已固化；开工前还需处理影响代码承诺的已知缺口，README 真实 GIF 可随 v0.1.1 补充。
...
- 本轮验证：自动更新 v0.0.3 → v0.1.0 端到端闭环已完成；手动检查结果弹窗通过组件测试与 TypeScript 检查，用户可见变化已写入 `CHANGELOG.md`。
...
1. 决定并补齐三类代码缺口：带 passphrase 私钥的 `connection_test`；`ChannelDropped` / `AcceptLoopDied` 的运行时检测与上报；安全传递 MySQL 行号的结构化错误契约。
2. 其余 V2-CP0 条件完成后启动 v0.2 Week 1；应用内升级闭环和 PostgreSQL 版本基线已完成。
3. 补 README “右键打开”与多跳拓扑真实 GIF，可随 v0.1.1 发布。
